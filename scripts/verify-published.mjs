...
if (!registryOnly) {
  ...
  for (const file of ["catalog.json", "catalog.sig", "catalog-v2.json", "catalog-v2.sig", manifest.artifact.file, ...manifest.reports.map(r => r.file), "manifest.json"]) {
    ...
    const local = file.startsWith("catalog") ? `assets/${file}` : file.endsWith(".tgz") ? `.release/package/${file}` : `.release/evidence/${file}`;
