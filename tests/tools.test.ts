...
describe("API worker tool harness", () => {
  ...
  it("rejects path escape and protected files", async () => {
    ...
    await expect(tools.execute("read_file", { path: "../secret" })).rejects.toThrow("상위 경로");
    await expect(tools.execute("write_file", { path: ".env", content: "SECRET=x", expectedSha256: null })).rejects.toThrow("보호된 경로");
  });

  it("uses SHA guards and can roll back partial writes", async () => {
    const tools = new WorkspaceTools(root, ["git diff --check"]);
    await tools.execute("edit_file", { path: "hello.txt", expectedSha256: sha256("hello\n"), oldText: "hello", newText: "world", expectedOccurrences: 1 });
    expect(await readFile(join(root, "hello.txt"), "utf8")).toBe("world\n");
    await expect(tools.execute("edit_file", { path: "hello.txt", expectedSha256: sha256("hello\n"), oldText: "world", newText: "x", expectedOccurrences: 1 })).rejects.toThrow("변경되었습니다");
