...
const criterionIds = [
  "contract_evidence", "deterministic_verification", "regression_scope_safety", "reproducibility",
  "api_contract_correctness", "data_business_integrity", "error_security_handling", "integration_evidence",
];
const gateIds = [
  "worker_execution_failed", "deterministic_verifier_failed", "secret_or_user_data_exposure",
  "core_placeholder_claimed_complete", "tests_weakened", "destructive_out_of_scope_change", "core_contract_broken",
];
let text = "";
if (request.role === "contractPlanner") {
  ...
  await writeFile("ralph-smoke.txt", "ok\n");
  text = "ralph-smoke.txt를 작성했습니다.";
}
process.stdout.write(`${JSON.stringify({ text, exitCode: 0, usage: { inputTokens: 10, outputTokens: 5, totalTokens: 15 } })}\n`);
