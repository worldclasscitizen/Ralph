...
describe("orchestration state machine", () => {
  it("uses an independent contract critic before returning a draft", async () => {
    ...
    const routes = Object.fromEntries(["planning_architecture","frontend_visual","backend_core","tdd_debugging","static_review","delivery_evidence","contractPlanner","router","critic","metaPrompter","worker","adjudicator"].map((key) => [key, [route]])) as ProjectConfig["routes"];
    await saveConfig(root, { schemaVersion: 1, projectRoot: root, preset: "balanced", initializedAt: new Date().toISOString(), connections: [{ id: "mock:process", adapter: "generic-process", provider: "mock", enabled: true, mode: "process", command: [process.execPath, fixture] }], routes, overrides: {}, verifierCommands: ["git diff --check"], catalogVersion: 2 });
