...
Mock 테스트는 유료 모델을 호출하지 않습니다. 실제 출시 검증은 명시적으로 실행하며 누적 호출 횟수는 제한하지 않습니다. 활성 시간 합계 30분과 추가 유료 API 연결 사용 금지는 유지합니다. 모든 호출·실패·취소를 기록하고 재실행으로 소비량을 초기화하지 않습니다. [출시 검증](./docs/project/v0.3-readiness.md)
